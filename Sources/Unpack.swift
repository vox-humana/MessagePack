...
func unpackInteger<G: IteratorProtocol where G.Element == Byte>(with iterator: inout G, size: Int) throws -> UInt64 {
    var value: UInt64 = 0
    for _ in 0..<size {
...
func unpackArray<G: IteratorProtocol where G.Element == Byte>(with iterator: inout G, count: Int, compatibility: Bool) throws -> [MessagePackValue] {
    var values = [MessagePackValue]()
    values.reserveCapacity(count)

    for _ in 0..<count {
        let value = try unpack(with: &iterator, compatibility: compatibility)
        values.append(value)
    }
...
func unpackMap<G: IteratorProtocol where G.Element == Byte>(with iterator: inout G, count: Int, compatibility: Bool) throws -> [MessagePackValue : MessagePackValue] {
    var dict = [MessagePackValue : MessagePackValue](minimumCapacity: count)
    var lastKey: MessagePackValue? = nil

    let array = try unpackArray(with: &iterator, count: 2 * count, compatibility: compatibility)
    for item in array {
        if let key = lastKey {
            dict[key] = item
            lastKey = nil
...
public func unpack<G: IteratorProtocol where G.Element == Byte>(with iterator: inout G, compatibility: Bool = false) throws -> MessagePackValue {
    if let value = iterator.next() {
        ...
        case 0x80...0x8f:
            ...
            let dict = try unpackMap(with: &iterator, count: count, compatibility: compatibility)
            return .Map(dict)

        // fixarray
        case 0x90...0x9f:
            let count = Int(value - 0x90)
            let array = try unpackArray(with: &iterator, count: count, compatibility: compatibility)
            return .Array(array)
        ...
        case 0xa0...0xbf:
            ...
            if compatibility {
                let data = try unpackData(with: &iterator, length: length)
                return .Binary(data)
            } else {
                let string = try unpackString(with: &iterator, length: length)
        ...
        case 0xc4...0xc6:
            ...
            let length = try unpackInteger(with: &iterator, size: size)
            let subdata = try unpackData(with: &iterator, length: numericCast(length))
        ...
        case 0xc7...0xc9:
            ...
            if let typeByte = iterator.next() {
                ...
                let data = try unpackData(with: &iterator, length: numericCast(length))
        ...
        // float 32
        case 0xca:
            let bytes = try unpackInteger(with: &iterator, size: 4)
            let float = unsafeBitCast(UInt32(truncatingBitPattern: bytes), to: Float.self)
            return .Float(float)

        // float 64
        case 0xcb:
            let bytes = try unpackInteger(with: &iterator, size: 8)
        ...
        case 0xcc...0xcf:
            ...
            let integer = try unpackInteger(with: &iterator, size: size)
            return .UInt(integer)
        ...
        // int 16
        case 0xd1:
            let bytes = try unpackInteger(with: &iterator, size: 2)
        ...
        case 0xd4...0xd8:
            ...
            if let typeByte = iterator.next() {
                let type = Int8(bitPattern: typeByte)
                let bytes = try unpackData(with: &iterator, length: length)
        ...
        case 0xd9...0xdb:
            ...
            let length = try unpackInteger(with: &iterator, size: lengthSize)
            if compatibility {
                let data = try unpackData(with: &iterator, length: numericCast(length))
                return .Binary(data)
            } else {
                let string = try unpackString(with: &iterator, length: numericCast(length))
                return .String(string)
            }
        ...
        case 0xdc...0xdd:
            ...
            let count = try unpackInteger(with: &iterator, size: countSize)
            let array = try unpackArray(with: &iterator, count: numericCast(count), compatibility: compatibility)
            return .Array(array)
        ...
        case 0xde...0xdf:
            ...
            let count = try unpackInteger(with: &iterator, size: countSize)
            let dict = try unpackMap(with: &iterator, count: numericCast(count), compatibility: compatibility)
            return .Map(dict)
